...
class Cpdf {
	...
	*/
	function Cpdf ($pageSize=array(0,0,612,792)){
		...
		$this->strCaminhoFontes = $SISCONF['SIS']['HOST'] . "/modulos/temas/fonts/"; #$SISCONF['SIS']['COMUM']['TEMAS'] . "fonts/";
	...
	*/
	function o_destination($id, $action, $options='') {
		if ($action!='new') {$o =& $this->objects[$id];}
	...
	*/
	function setCurrentFont(){
		if (strlen($this->currentBaseFont)==0){
			...
			$this->selectFont($this->strCaminhoFontes.'Helvetica.afm');
	  	}
	...
	*/
	function stream($options='', $gera_arq=false){
		...
		if ($gera_arq == false) {
			...
			echo ltrim($tmp);
		} elseif ($gera_arq == true) {
			return trim($tmp);;
		}	  
	}
	
	function paraArq($options=''){
		// setting the options allows the adjustment of the headers
		// values at the moment are:
		// 'Content-Disposition'=>'filename'  - sets the filename, though not too sure how well this will 
		//        work as in my trial the browser seems to use the filename of the php file with .pdf on the end
		...
		$arq = new geracaoArquivo();
		$arq->_file = "jfz".$_REQUEST['PHPSESSID']; 
		$arq->_file_extension = "pdf";
		$arq->filename;
		$arq->noDateInFile = true;
		$arq->makeFilename();
		$arq->openFile();  
	...
	*/
	function getFontHeight($size){
		if (!$this->numFonts){
			...
			$this->selectFont($this->strCaminhoFontes.'Helvetica');
		}
	...
	*/
	function addText($x,$y,$size,$text,$angle=0,$wordSpaceAdjust=0){
		if (!$this->numFonts){$this->selectFont($this->strCaminhoFontes.'Helvetica');}
		//if (!$this->numFonts){$this->selectFont('../temas/fonts/Helvetica');}
